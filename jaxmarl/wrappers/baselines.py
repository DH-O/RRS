...
class LogWrapper(JaxMARLWrapper):
    ...
    def reset(self, key: chex.PRNGKey) -> Tuple[chex.Array, State]:
        ...
        state = LogEnvState(
            env_state,
            jnp.zeros((len(self._env.agents),)),
            jnp.zeros((len(self._env.agents),)),
            jnp.zeros((len(self._env.agents),)),
            jnp.zeros((len(self._env.agents),)),
        )
    ...
    def step(
        ...
        info["returned_episode"] = jnp.full((len(self._env.agents),), ep_done)
...
class SMAXLogWrapper(JaxMARLWrapper):
    """Log episode returns, lengths, AND win rate for SMAX environments.
    won_episode is detected by checking if any step reward >= 1.0 (won_battle_bonus).
    """
    ...
    def step(
        ...
        state: SMAXLogEnvState,
        action: Union[int, float],
    ) -> Tuple[chex.Array, SMAXLogEnvState, float, bool, dict]:
        obs, env_state, reward, done, info = self._env.step(
            key, state.env_state, action
        )
        ep_done = done["__all__"]
        batch_reward = self._batchify_floats(reward)
        new_episode_return = state.episode_returns + batch_reward
        new_episode_length = state.episode_lengths + 1
        # Detect win: won_battle_bonus=1.0 is added when all enemies are dead
        new_won_episode = jnp.maximum(state.won_episode, (batch_reward >= 1.0).astype(jnp.float32))
        state = SMAXLogEnvState(
            env_state=env_state,
...
class MPELogWrapper(LogWrapper):
    ...
    def step(
        ...
        rewardlog = jax.tree.map(lambda x: x*len(self._env.agents), reward)  # As per on-policy codebase
        ep_done = done["__all__"]
