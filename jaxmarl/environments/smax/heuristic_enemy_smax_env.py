...
class EnemySMAX(MultiAgentEnv):
    ...
    # Subclasses can override this to use a different base env class
    _env_class = SMAX

    def __init__(self, **env_kwargs):
        self._env = self._env_class(**env_kwargs)
        # only one team
...
class HeuristicEnemyCorridorSMAX(HeuristicEnemySMAX):
    """HeuristicEnemySMAX that uses SMAXCorridor as the base environment.

    This provides the same heuristic enemy behavior but with wall obstacles.
    """

    @property
    def _env_class(self):
        # Lazy import to avoid circular dependency
        from jaxmarl.environments.smax.smax_corridor_env import SMAXCorridor
        return SMAXCorridor

    def __init__(self, enemy_shoots=True, attack_mode="closest", **env_kwargs):
        # Cannot use class variable _env_class because we need lazy import.
        # Override __init__ to manually set self._env.
        from jaxmarl.environments.smax.smax_corridor_env import SMAXCorridor
        # Skip EnemySMAX.__init__ and do it manually with SMAXCorridor
        self._env = SMAXCorridor(**env_kwargs)
        self.num_agents = self._env.num_allies
        self.num_enemies = self._env.num_enemies
        self.num_allies = self._env.num_allies
        self.agents = [f"ally_{i}" for i in range(self.num_agents)]
