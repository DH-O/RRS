...
MAP_NAME_TO_SCENARIO = {
    ...
    "3m": Scenario(np.zeros((6,), dtype=np.uint8), 3, 3, False, False),
    "2s3z": Scenario(
        np.array([2, 2, 3, 3, 3] * 2, dtype=np.uint8), 5, 5, False, False
    ),
    "25m": Scenario(np.zeros((50,), dtype=np.uint8), 25, 25, False, False),
    "3s5z": Scenario(
        np.array(
            [
                2,
    ...
    "8m": Scenario(np.zeros((16,), dtype=np.uint8), 8, 8, False, False),
    "5m_vs_6m": Scenario(np.zeros((11,), dtype=np.uint8), 5, 6, False, False),
    "10m_vs_11m": Scenario(np.zeros((21,), dtype=np.uint8), 10, 11, False, False),
    "27m_vs_30m": Scenario(np.zeros((57,), dtype=np.uint8), 27, 30, False, False),
    "3s5z_vs_3s6z": Scenario(
        np.concatenate(
            [
                np.array([2, 2, 2, 3, 3, 3, 3, 3], dtype=np.uint8),
                np.array([2, 2, 2, 3, 3, 3, 3, 3, 3], dtype=np.uint8),
            ]
        ),
        8,
        9,
    ...
    "3s_vs_5z": Scenario(
        np.array([2, 2, 2, 3, 3, 3, 3, 3], dtype=np.uint8), 3, 5, False, False
    ),
    "6h_vs_8z": Scenario(
        np.array([5, 5, 5, 5, 5, 5, 3, 3, 3, 3, 3, 3, 3, 3], dtype=np.uint8),
        6,
        8,
        False,
        False,
    ),
    "smacv2_5_units": Scenario(np.zeros((10,), dtype=np.uint8), 5, 5, True, True),
    "smacv2_10_units": Scenario(np.zeros((20,), dtype=np.uint8), 10, 10, True, True),
    "smacv2_20_units": Scenario(np.zeros((40,), dtype=np.uint8), 20, 20, True, True),
    "12h_vs_16z": Scenario(
        np.array([5] * 12 + [3] * 16, dtype=np.uint8), 12, 16, False, False
    ),
    "6s10z_vs_6s12z": Scenario(
        np.concatenate(
            [
                np.array([2] * 6 + [3] * 10, dtype=np.uint8),
                np.array([2] * 6 + [3] * 12, dtype=np.uint8),
            ]
        ),
        16,
        18,
...
class SMAX(MultiAgentEnv):
    def __init__(
        ...
        unit_type_velocities=np.array([3.15, 2.25, 4.13, 3.15, 4.13, 3.15]),
        unit_type_attacks=np.array([9.0, 10.0, 13.0, 8.0, 5.0, 12.0]),
        unit_type_attack_ranges=np.array([5.0, 6.0, 6.0, 2.0, 2.0, 5.0]),
        unit_type_sight_ranges=np.array([9.0, 10.0, 10.0, 9.0, 8.0, 9.0]),
        unit_type_radiuses=np.array([0.375, 0.5625, 0.625, 0.5, 0.375, 0.625]),
        unit_type_health=np.array([45.0, 125.0, 160, 150, 35, 80]),
        unit_type_weapon_cooldowns=np.array([0.61, 1.07, 1.87, 0.86, 0.5, 0.59]),
        ...
        self.scenario = scenario if scenario is None else jnp.asarray(scenario.unit_types)
        self.use_self_play_reward = use_self_play_reward
        self.time_per_step = time_per_step
        self.unit_type_velocities = jnp.asarray(unit_type_velocities)
        self.unit_type_weapon_cooldowns = jnp.asarray(unit_type_weapon_cooldowns)
        self.unit_type_attacks = jnp.asarray(unit_type_attacks)
        self.unit_type_attack_ranges = jnp.asarray(unit_type_attack_ranges)
        self.unit_type_sight_ranges = jnp.asarray(unit_type_sight_ranges)
        self.unit_type_radiuses = jnp.asarray(unit_type_radiuses)
        self.unit_type_health = jnp.asarray(unit_type_health)
