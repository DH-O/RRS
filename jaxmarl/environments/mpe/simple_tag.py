...
class SimpleTagMPE(SimpleMPE):
    def __init__(
        ...
        **kwargs,
        ...
        # Adversary obs: vel(2) + pos(2) + landmarks(n_obs*2) + others((n_agents-1)*2) + last_vel(2)
        # Good agent obs: vel(2) + pos(2) + landmarks(n_obs*2) + others((n_agents-1)*2) (no last_vel)
        adv_obs_dim = 6 + num_landmarks * 2 + (num_agents - 1) * 2
        good_obs_dim = 4 + num_landmarks * 2 + (num_agents - 1) * 2
        observation_spaces = {
            i: Box(-jnp.inf, jnp.inf, (adv_obs_dim,)) for i in self.adversaries
        }
        observation_spaces.update(
            {i: Box(-jnp.inf, jnp.inf, (good_obs_dim,)) for i in self.good_agents}
        )
        ...
        max_speed = jnp.concatenate(
            [
                ...
                jnp.full((self.num_good_agents), prey_speed),
