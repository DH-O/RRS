...
class AntBall(MABraxEnv):
    """Ant with ball-on-tray. Uses custom Brax env instead of registry."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        from .ant_ball_env import AntBall as AntBallBrax

        env_name = "ant_ball_4x2"
        env = AntBallBrax(backend=backend, episode_length=episode_length,
                          action_repeat=action_repeat, auto_reset=auto_reset,
                          **kwargs)
        self.env = env
        self.episode_length = episode_length
        self.action_repeat = action_repeat
        self.auto_reset = auto_reset
        self.homogenisation_method = None
        self.agent_obs_mapping = _agent_observation_mapping[env_name]
        self.agent_action_mapping = _agent_action_mapping[env_name]
        self.agents = list(self.agent_obs_mapping.keys())
        self.num_agents = len(self.agent_obs_mapping)

        obs_sizes = {agent: obs.size for agent, obs in self.agent_obs_mapping.items()}
        act_sizes = {agent: act.size for agent, act in self.agent_action_mapping.items()}
